...
module Tvdbr
  class Client
    ...
    def find_all_series_by_title(title)
      result = self.class.get("/GetSeries.php", :query => { :seriesname => title })['Data']
      return [] if result.blank? || result['Series'].blank?
      result = result['Series'].is_a?(Array) ? result['Series'] : [result['Series']]
      result.map { |s| self.find_series_by_id(s['seriesid']) }
    ...
    def find_series_by_title(title)
      self.find_all_series_by_title(title)
    end
    ...
    def actor_urls(series_id)
      self.get_with_key("/series/#{series_id}/actors.xml")['Actors']['Actor']
    end
    
    def banners_temporada(numero,serie_id)
      
      banners = []
      
      for hash in self.banner_urls(serie_id)
        if  hash["BannerType"].eql? "season"
          if hash["BannerType2"].eql? "season"
            if numero.to_s.eql? hash["Season"]
              banners<< "http://www.thetvdb.com/banners/#{hash["BannerPath"]}"
            end
          end 
        end
      end
    ...
    protected
      ...
      def check_api_key!
        ...
        raise "Please check your TVDB API Key, '#{self.api_key}' seems invalid! #{e}"
      end
  end
end
